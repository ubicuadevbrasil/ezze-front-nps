...
>(
  (
    ...
    return (
			<div ref={ref} className={cn('grid min-w-[8rem] items-start gap-1.5 rounded-md -lg border border-border/50 bg-background px-2.5 py-1.5 text-xs shadow-xl', className)}>
								...
								{formatter && item?.value !== undefined && item.name ? (
									...
									<>
										{itemConfig?.icon ? (
											...
											!hideIndicator && (
												<div
													className={cn('shrink-0 rounded-md -[2px] border-[--color-border] bg-[--color-bg]', {
          ...
          return (
					<div key={item.value} className={cn('flex items-center gap-1.5 [&>svg]:h-3 [&>svg]:w-3 [&>svg]:text-muted-foreground')}>
						{itemConfig?.icon && !hideIcon ? (
							...
							<div
								className="h-2 w-2 shrink-0 rounded-md -[2px]"
								style={{
