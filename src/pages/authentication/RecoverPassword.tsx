...
const RecoverPassword: React.FC = () => {
	...
	return (
		<>
			<Link to={'/'} className="flex gap-2.5 self-start items-center mb-3 py-2 px-3 rounded bg-[#D9DDE5]">
			...
			<form onSubmit={handleSubmit(handleSendCodeToContact)} action="" className="w-full flex flex-col gap-4 mt-4">
				<div className="w-full flex gap-2.5">
					<label htmlFor="email" className={`w-full flex justify-center py-2 border rounded ${selectedValue === 'email' ? 'bg-[#1E3868] text-white border-[#6384D3]' : ''}`}>
					...
					<label htmlFor="sms" className={`w-full flex justify-center py-2 border rounded ${selectedValue === 'sms' ? 'bg-[#1E3868] text-white border-[#6384D3]' : ''}`}>
				...
				<button disabled={isSubmitDisable} className="w-full flex justify-center py-2 bg-[#365DA5] text-white rounded disabled:opacity-40">
